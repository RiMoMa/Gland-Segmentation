...
def gland_predictions(files_path=None,patchsize=(512,512), batch_size=100,probValueF=0.5,path_svs = None, allGpus=False,ImgNormalize=False, MetaplasiaPredsCase=None,CaseName=None,OutputOverlay=None, OutputGlandPath=None):
    ...
    with (torch.no_grad()):
        ...
        for total_pred,img,tileName in zip(total_preds,imgs,tileInfo):
...
# Uso de la función
ExpProbabilities = [2.5,0.96,0.9]
for ExpProbability in ExpProbabilities:
    print(ExpProbability)
    probValue = ExpProbability
    path_folder = '/media/ricardo/Datos/Data/Processed_Patches/annotated/'
    OutputOverlayGland = '/media/ricardo/Datos/Data/Processed_Patches/annotated_GlandPredictionOverlay'+str(ExpProbability)+'/'
    OutputGlandPath = '/media/ricardo/Datos/Data/Processed_Patches/annotated_GlandDatasetNormalized'+str(ExpProbability)+'/'
    patchsize = (512 *2*4, 512*2 * 4)
    batch_size = 50
    HighResol = False
    ImgNormalize = True
