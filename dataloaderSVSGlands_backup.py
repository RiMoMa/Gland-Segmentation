...
class DataLoader(DataLoader):
    def __init__(self, data, svs_path,ImgNormalize,CaseName, batch_size, patch_size, num_threads_in_multithreaded,
    ...
    def generate_train_batch(self):
        ...
        if self.ImgNormalize==False:
            ...
            input_path = PathDataset+'*.png'
            output_path = 'Dataset_Normalized/' + CaseName + '/'
            ...
            for i, (nombre, (x, y)) in enumerate(gland_img):
                tileInfo.append(nombre)
                tileCoords.append((x, y))
                img_data = self.load_patient_imNormalized(slide, x, y, self.patch_size[0], nombre, output_path)
                # hence we use tensor manipulation to convert to channel first
                # dummy dimension in order for it to work (@Todo, could be improved)
                img_data = np.einsum('hwc->chw', img_data)
                if self.crop_status:
