...
import os
def generate_WSI(patchsize=(512,512), batch_size=100, path_svs = None,outputIm=None,HighResol=False, allGpus=False,ImgNormalize=False,MetaplasiaPredsCase=None,CaseName=None,inputDataset=None):
    # open svs
    ...
    with torch.no_grad():
        for i in tqdm(range(num_batches)):
            ...
            total_preds = repeat_channel(swap(np.where(np.vstack(tuple([pred_list])) > 0.7, 1.0, 0.0)))
            tileInfo = val_batch['tileInfo']
            tileCoords = val_batch['tileCoords']
#            imgs = imgs.cpu().detach().numpy()
            imgs = val_batch['data']
            PathGlandsMetaplasia = inputDataset+'/Dataset_GlandDetection/MetaplasiaGland/' + CaseName + '/'
            os.makedirs(PathGlandsMetaplasia, exist_ok=True)

            PathGlandsControl = inputDataset+'/Dataset_GlandDetection/ControlGland/' + CaseName + '/'
...
import subprocess
def process_all_images(path_folder, patchsize, batch_size, HighResol, ImgNormalize,OutputFolder):
    # Listar todos los archivos en la carpeta
    all_files = os.listdir(path_folder)

    # Filtrar solo archivos svs
    svs_files = [file for file in all_files if file.endswith('.png')]
    start_from = ''
    if start_from == '':
        svs_files_to_process = svs_files
    else:

        # Encuentra el índice del archivo para comenzar
        start_index = svs_files.index(start_from)

        # subconjunto de la lista de archivos para procesar desde el archivo deseado en adelante
        svs_files_to_process = svs_files[start_index:]

    # Procesar cada archivo
    for svs_file in svs_files_to_process:#svs_files:
        #svs_file = '001_194_2019_he.svs' #todo: borrar esto
        path_svs = os.path.join(path_folder, svs_file)
        outputIm = svs_file.replace('.png', '.jpc')

        # La última parte toma en cuenta solo el nombre del archivo
        case_name = os.path.basename(svs_file).replace('.png', '')
       ...
       # MetaplasiaPredsCase = MetaplasiaPreds.loc[case_name]
        ...
        ####################################################################
        FolderOutSVS = OutputDataset+'/PredictedSVS/'
        import functionPatchesExtraction
        functionPatchesExtraction.generate_Patches(patchsize, batch_size, path_svs, outputIm=outputIm, HighResol=HighResol,
                         ImgNormalize=ImgNormalize, CaseName=case_name,OutputPath=OutputDataset)

        input_path = OutputDataset+case_name+ '/*.png'
        output_path = OutputDataset+'Dataset_Normalized/' + case_name + '/'

        input_files = glob.glob(input_path)
        output_files = glob.glob(output_path + "*.png")
        ...
        # Verifica si los nombres de los archivos coinciden en ambos directorios
        if set(input_files) == set(output_files):
            print("Los archivos coinciden.")
        else:
        ...
        generate_WSI(patchsize, batch_size, path_svs, outputIm=FolderOutSVS+'/'+outputIm, HighResol=HighResol, ImgNormalize=ImgNormalize,
                 MetaplasiaPredsCase=MetaplasiaPredsCase,
                 CaseName=case_name,
                 inputDataset= OutputDataset)

        # Recorre la lista de archivos y los borra
        for file in output_files:
            os.remove(output_path+file)


# Uso de la función
path_folder = '/media/ricardo/Datos/Regiones/'
OutputDataset = '/media/ricardo/Datos/RegionesProcesadas/'
patchsize = (512*4, 512*4)
batch_size = 50
HighResol = False
ImgNormalize = True

process_all_images(path_folder, patchsize, batch_size, HighResol, ImgNormalize,OutputFolder=OutputDataset)
